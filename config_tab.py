...
class ConfigTab(BaseTab):
    ...
    def _test_google_sheets(self):
        """Prueba conexión Google Sheets (ahora pasa la ruta de credenciales al backend)."""
        ...
        if not creds_file:
            self.show_warning("Google Sheets", "Ingresá la ruta/al nombre del archivo de credenciales (credentials.json o service_account.json).")
            return

        # Guardar temporalmente en settings para que el backend lo lea (y también persitir si querés)
        try:
            settings.set("google_sheets.sheet_key", sheet_key)
            settings.set("google_sheets.credentials_file", creds_file)
        except Exception:
            # Si settings no soporta set(), ignoramos (pero lo intentamos)
            pass
        ...
        def worker():
            ...
            try:
                ...
                # Manejo local de errores para dar instrucciones más útiles en la UI
                errmsg = str(e)
                if "Client secrets must be for a web or installed app" in errmsg:
                    user_msg = (
                        "El archivo de credenciales que seleccionaste parece ser un OAuth client_secrets\n"
                        "pero el backend espera un Service Account, o viceversa.\n\n"
                        "Soluciones:\n"
                        "- Si querés que la app sincronice automáticamente, usa un Service Account JSON\n"
                        "  (creá una service account en Google Cloud y descarga la key JSON). Luego compartí\n"
                        "  la hoja con el email de la service account.\n"
                        "- Si querés autorizar con una cuenta humana, crea un OAuth Client ID tipo 'Desktop' o 'Web'\n"
                        "  y seleccioná ese client_secrets.json; se abrirá un navegador para autorizar.\n\n"
                        "Asegurate además de que el campo 'Archivo de Credenciales' contenga la ruta correcta\n"
                        "al JSON que descargaste y que el archivo exista en esa ruta."
                    )
                    ok = False
                    msg = user_msg
                else:
                    ok = False
                    msg = f"Error probando conexión: {errmsg}"
